...
class MainWindow(tk.Tk):
    ...
    def _create_url_section(self, parent):
        ...
        self.url_entry = tk.Text(
            url_frame,
            font=StyleManager.default_font(12),
            bg=Colors.SECONDARY,
            fg=Colors.TEXT_PRIMARY,
        ...
        self.url_entry.grid(row=0, column=1, sticky="ew", padx=10, ipadx=15, pady=2)
    ...
    def _create_download_section(self, parent):
        ...
        # Progress Bar
        self.progress_bar = AnimatedProgressBar(download_frame, width=600, height=8)
    ...
    def _set_icon(self):
        ...
        try:
            ...
            photo_path = Config.get_resource_path("yuload/assets/logo.ico")
            print(photo_path, "ICON PATH")
    ...
    def _on_load_click(self):
        ...
        # Récupération de l'URL depuis Text widget
        ...
        threading.Thread(
            target=self._load_video_thread, args=(url,), daemon=True
        ).start()
        ...
        try:
            ...
            if not video_info:
                self.status_bar.show_message(
                    "Failed to load video. Check URL or internet connection.", "error"
                )
                self.is_loading = False
            ...
            if video_info:  # Si chargement réussi
                ...
                self.after(
                    2000, self.loading_spinner.grid_remove
    ...
    def _on_download_click(self):
        ...
        if not self.current_video_info:
            self.status_bar.show_message(
                "S'il vous plait, chargez d'abord une vidéo", "warning"
            )
            return

        if not self.selected_quality:
            self.status_bar.show_message(
                "Veuillez d'abord selectionner la qualité", "warning"
            )
            return

        self.output_folder: str = filedialog.askdirectory(
            title="Selectionnez le dossier de téléchargement",
        ...
        include_subtitle: bool = self.subtitle_selector.should_include()
        subtitle_code: str | None = (
            self.subtitle_selector.get_selected_code() if include_subtitle else None
        )
    ...
    def _on_download_complete(self, filepath: str):
        ...
        self.status_bar.show_message(
            f"Download completed: {Path(filepath).name}", "success"
        )

        # Restauration de l'écran après 2 sec
    ...
    def _on_browse_folder(self):
        ...
        folder = self.output_folder if hasattr(self, "output_folder") else str(Config.DOWNLOAD_DIR)
        try:
            import subprocess
            import sys
            ...
            self.status_bar.show_message(
                "Impossible d'ouvrir le dossier. Vous pouvez l'ouvrir manuellement.",
                "error",
            )
