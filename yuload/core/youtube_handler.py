...
class YouTubeHandler:
    ...
    def get_available_streams(self) -> Optional[List[Dict]]:
        ...
        try:
            streams = self.current_video.streams.filter(
            ...
            for stream in sorted(
                valid_streams, key=lambda x: int(x.resolution[:-1]), reverse=True
            ):
            ...
            msg: str = (
                ...
                else "Nous n'avons pas pu recupérer toute la vidéo. Veuillez vérifiez votre connexion puis réessayez."
            )
    ...
    def get_stream_by_quality(self, quality: str):
        ...
        try:
            stream = self.current_video.streams.filter(
                progressive=False, file_extension="mp4", resolution=quality
            ).first()
            return stream
        except Exception as e:
